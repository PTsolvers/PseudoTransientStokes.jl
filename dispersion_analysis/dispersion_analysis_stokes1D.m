...
syms p(tau,x) v_x(tau,x) tau_xx(tau,x)
syms K G rho eta k Lx Re r v0 positive
syms lambda_k
%% governing equations
eq1        = 1/K*diff(p(tau,x)     ,tau) + diff(            v_x(tau,x),x);             % mass balance
eq2        = rho*diff(v_x(tau,x)   ,tau) + diff(p(tau,x)-tau_xx(tau,x),x);             % momentum balace
eq3        = 1/G*diff(tau_xx(tau,x),tau) + 1/eta*tau_xx(tau,x) - 2*diff(v_x(tau,x),x); % Maxwell rheological model
%% equation for velocity
tmp        = diff(eq2,tau)/G + diff(eq3,x) + eq2/eta - diff(eq1,x)*K/G;
eq_v       = expand(diff(tmp,tau)*eta - diff(eq1,x)*K);
...
rho        = solve(Re == rho*V_p*Lx/eta,rho);                                          % density from Reynolds number Re
%% dispersion relation
v_x(tau,x) = v0*exp(-lambda_k*V_p*tau/Lx)*sin(pi*k*x/Lx);                              % Fourier term
disp_rel   = subs(subs(eq_v/v_x(tau,x)));
