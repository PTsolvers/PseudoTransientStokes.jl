...
@parallel function compute_iter_params!(RhoG::Data.Array, dt_Rho::Data.Array, Gdt::Data.Array, Musτ::Data.Array, CFL::Data.Number, G::Data.Number, dt::Data.Number, Re2::Data.Number, min_dxyz::Data.Number, max_lxyz2::Data.Number)
    @all(RhoG)   = Re2*(@all(Musτ)/(@all(Musτ)/(G*dt)+1))^2/max_lxyz2
    @all(dt_Rho) = CFL*min_dxyz/sqrt(@all(RhoG))
...
@parallel function compute_P!(∇V::Data.Array, Pt::Data.Array, Vx::Data.Array, Vy::Data.Array, Vz::Data.Array, Gdt::Data.Array, K_G::Data.Number, dx::Data.Number, dy::Data.Number, dz::Data.Number)
...
@parallel function compute_τ!(τxx::Data.Array, τyy::Data.Array, τzz::Data.Array, τxy::Data.Array, τxz::Data.Array, τyz::Data.Array, τxx_o::Data.Array, τyy_o::Data.Array, τzz_o::Data.Array, τxy_o::Data.Array, τxz_o::Data.Array, τyz_o::Data.Array, Vx::Data.Array, Vy::Data.Array, Vz::Data.Array, Mus::Data.Array, ∇V::Data.Array, Gdt::Data.Array, G::Data.Number, dt::Data.Number, dx::Data.Number, dy::Data.Number, dz::Data.Number)
    @all(τxx) = (@all(τxx) + @all(τxx_o)*@inn_yz_Gr() + 2.0*@inn_yz(Gdt)*(@d_xi(Vx)/dx - 1.0/3.0*@inn_yz(∇V)))/(1.0 + @inn_yz(Gdt)/@inn_yz(Mus) + @inn_yz_Gr())
    @all(τyy) = (@all(τyy) + @all(τyy_o)*@inn_xz_Gr() + 2.0*@inn_xz(Gdt)*(@d_yi(Vy)/dy - 1.0/3.0*@inn_xz(∇V)))/(1.0 + @inn_xz(Gdt)/@inn_xz(Mus) + @inn_xz_Gr())
    @all(τzz) = (@all(τzz) + @all(τzz_o)*@inn_xy_Gr() + 2.0*@inn_xy(Gdt)*(@d_zi(Vz)/dz - 1.0/3.0*@inn_xy(∇V)))/(1.0 + @inn_xy(Gdt)/@inn_xy(Mus) + @inn_xy_Gr())
...
@views function Stokes3D()
    ...
    Re2        = (5.0π)^2  #(2.3π)^2  # Reynolds number squared
    K_G        = 0.5       #1.0       # Bulk to shear elastic modulus ratio
    CFL        = 1/3       #1/2.5     # CFL number
    ...
    min_dxyz   = min(dx,dy,dz)
    max_lxyz2  = max(lx,ly,lz)^2
    # Array allocations
    Pt         = @zeros(nx  ,ny  ,nz  )
    RhoG       = @zeros(nx  ,ny  ,nz  )
    ...
    for it = 1:nt
        ...
        while err > ε && iter <= iterMax
            ...
            @parallel compute_P!(∇V, Pt, Vx, Vy, Vz, Gdt, K_G, dx, dy, dz)
            @parallel compute_τ!(τxx, τyy, τzz, τxy, τxz, τyz, τxx_o, τyy_o, τzz_o, τxy_o, τxz_o, τyz_o, Vx, Vy, Vz, Mus, ∇V, Gdt, G, dt, dx, dy, dz)
    ...
    if me==0 && do_save
        ...
        open("../../output/out_Stokes3D_ve2.txt","a") do io
