...
@parallel function compute_timesteps!(dτVx::Data.Array, dτVy::Data.Array, dτVz::Data.Array, dτPt::Data.Array, Musτ::Data.Array, Vsc::Data.Number, Ptsc::Data.Number, min_dxyz2::Data.Number, β_n::Data.Number, max_nxyz)
    @all(dτVx) = 1.0/6.1/(1.0+β_n)/Vsc*min_dxyz2/@av_xi(Musτ)
    @all(dτVy) = 1.0/6.1/(1.0+β_n)/Vsc*min_dxyz2/@av_yi(Musτ)
    @all(dτVz) = 1.0/6.1/(1.0+β_n)/Vsc*min_dxyz2/@av_zi(Musτ)
    @all(dτPt) = 6.1*(1.0+β_n)/Ptsc/max_nxyz*@all(Musτ)
    return
end
...
macro inn_yz_Xsi() esc(:( @inn_yz(Mus)/(G*dt) )) end
macro inn_xz_Xsi() esc(:( @inn_xz(Mus)/(G*dt) )) end
macro inn_xy_Xsi() esc(:( @inn_xy(Mus)/(G*dt) )) end
macro av_xyi_Xsi() esc(:( @av_xyi(Mus)/(G*dt) )) end
macro av_xzi_Xsi() esc(:( @av_xzi(Mus)/(G*dt) )) end
macro av_yzi_Xsi() esc(:( @av_yzi(Mus)/(G*dt) )) end
@parallel function compute_τ!(τxx::Data.Array, τyy::Data.Array, τzz::Data.Array, τxy::Data.Array, τxz::Data.Array, τyz::Data.Array, τxx_o::Data.Array, τyy_o::Data.Array, τzz_o::Data.Array, τxy_o::Data.Array, τxz_o::Data.Array, τyz_o::Data.Array, Vx::Data.Array, Vy::Data.Array, Vz::Data.Array, Mus::Data.Array, ∇V::Data.Array, G::Data.Number, dt::Data.Number, β_n::Data.Number, dx::Data.Number, dy::Data.Number, dz::Data.Number)
    @all(τxx) = @all(τxx_o)*@inn_yz_Xsi()/(@inn_yz_Xsi()+1.0) + 2.0*@inn_yz(Mus)/(@inn_yz_Xsi()+1.0)*(@d_xi(Vx)/dx  - 1.0/3.0*@inn_yz(∇V) + β_n*@inn_yz(∇V))
    @all(τyy) = @all(τyy_o)*@inn_xz_Xsi()/(@inn_xz_Xsi()+1.0) + 2.0*@inn_xz(Mus)/(@inn_xz_Xsi()+1.0)*(@d_yi(Vy)/dy  - 1.0/3.0*@inn_xz(∇V) + β_n*@inn_xz(∇V))
    @all(τzz) = @all(τzz_o)*@inn_xy_Xsi()/(@inn_xy_Xsi()+1.0) + 2.0*@inn_xy(Mus)/(@inn_xy_Xsi()+1.0)*(@d_zi(Vz)/dz  - 1.0/3.0*@inn_xy(∇V) + β_n*@inn_xy(∇V))
    @all(τxy) = @all(τxy_o)*@av_xyi_Xsi()/(@av_xyi_Xsi()+1.0) + 2.0*@av_xyi(Mus)/(@av_xyi_Xsi()+1.0)*(0.5*(@d_yi(Vx)/dy + @d_xi(Vy)/dx))
    @all(τxz) = @all(τxz_o)*@av_xzi_Xsi()/(@av_xzi_Xsi()+1.0) + 2.0*@av_xzi(Mus)/(@av_xzi_Xsi()+1.0)*(0.5*(@d_zi(Vx)/dz + @d_xi(Vz)/dx))
    @all(τyz) = @all(τyz_o)*@av_yzi_Xsi()/(@av_yzi_Xsi()+1.0) + 2.0*@av_yzi(Mus)/(@av_yzi_Xsi()+1.0)*(0.5*(@d_zi(Vy)/dz + @d_yi(Vz)/dy))
    return
end
...
@views function Stokes3D()
    ...
    Ptsc       = 5.0               # relaxation paramter for the pressure equation pseudo-timestep limiter
    ...
    for it = 1:nt
        ...
        while err > ε && iter <= iterMax
            ...
            @parallel compute_τ!(τxx, τyy, τzz, τxy, τxz, τyz, τxx_o, τyy_o, τzz_o, τxy_o, τxz_o, τyz_o, Vx, Vy, Vz, Mus, ∇V, G, dt, β_n, dx, dy, dz)
    ...
    if (me==0) matwrite("Stokes_3D_ve.mat", Dict("Pt_3D"=> Pt_v, "Mus_3D"=> Mus_v, "Txz_3D"=> τxz_v, "Vz_3D"=> Vz_v, "dx_3D"=> dx, "dy_3D"=> dy, "dz_3D"=> dz); compress = true) end
    if me==0 && do_save
